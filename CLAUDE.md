...
## Project: Запись на звонок

Упрощённый сервис бронирования времени (Cal.com-like). Нет авторизации, один владелец, гости бронируют без аккаунта.
...
**Guest** (`/`): `GET /event-types`, `GET /event-types/{id}/slots?date=`, `POST /bookings`, `GET /booking-window`
**Owner** (`/owner/`): CRUD `/event-types`, `GET /bookings`
...
- ALWAYS run `make tsc` after changing frontend code
- ALWAYS run `make spec-compile` after changing `.tsp` files
- ALWAYS verify `make build` succeeds before committing
